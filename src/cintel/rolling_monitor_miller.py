"""
rolling_monitor_miller.py - Project script.
...
Paths (relative to repo root)
    ...
    OUTPUT FILE: artifacts/rolling_metrics_miller.csv
...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "rolling_metrics_miller.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 3.4: DEFINE ROLLING STD DEV FOR LATENCY
    # ----------------------------------------------------
    # The `total_latency_ms` column holds the total latency in milliseconds at each timestamp.
    latency_rolling_std_dev_recipe: pl.Expr = (
        pl.col("total_latency_ms")
        .rolling_std(WINDOW_SIZE)
        .alias("latency_rolling_std_dev")
    )
    ...
    df_with_rolling = df.with_columns(
        [
            ...
            latency_rolling_std_dev_recipe,
        ]
    )
